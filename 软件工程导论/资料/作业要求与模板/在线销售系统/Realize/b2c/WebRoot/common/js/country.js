...
function set(nation, province, city){ 
	var str="0"; 
	ar = dsy.Items[str];
	var point=0;
	var na=document.getElementById(s[0]);
	na[0]=new Option(opt0[0],opt0[0]); 
	for(i=1;i<=ar.length;i=i+1){
		na.options[i]=new Option(ar[i-1],ar[i-1]);
		if ( nation.indexOf(ar[i-1])!=-1 )
			point=i-1;
	}
	na.options[point].selected = true;
	
	var str="0_"+point;
	point=0;
	ar = dsy.Items[str];
	var pr=document.getElementById(s[1]);
	pr[0]=new Option(opt0[1],opt0[1]); 
	for(i=1;i<=ar.length;i=i+1){
		pr.options[i]=new Option(ar[i-1],ar[i-1]);
		if ( province.indexOf(ar[i-1])!=-1 )
			point=i-1;
	}
	pr.options[point].selected = true;

	
	str=str+"_"+point;
	point=0;
	ar = dsy.Items[str];
	var ci=document.getElementById(s[2]);
	ci[0]=new Option(opt0[2],opt0[2]); 
	for(i=1;i<=ar.length;i=i+1){
		ci.options[i]=new Option(ar[i-1],ar[i-1]);
		if ( city.indexOf(ar[i-1])!=-1 )
			point=i-1;
	}
	ci.options[point].selected = true;
...
dsy.add("0_0",["安徽","北京","福建","甘肃","广东","广西","贵州","海南","河北","河南","黑龙江","湖北","湖南","吉林","江苏","江西","辽宁","内蒙古","宁夏","青海","山东","山西","陕西","上海","四川","天津","西藏","新疆","云南","浙江","重庆","香港","澳门","台湾"]); 

dsy.add("0_0_0",["安庆","蚌埠","巢湖","池州","滁州","阜阳","合肥","淮北","淮南","黄山","六安","马鞍山","宿州","铜陵","芜湖","宣城","亳州"]); 
dsy.add("0_0_1",["东城","西城","宣武","崇文","朝阳","海淀","丰台","门头沟","房山","石景山区","通州","平觳","顺义","怀柔","密云","延庆","昌平","大兴"]); 
...
dsy.add("0_0_7",["白沙黎族自治县","保亭黎族苗族自治县","昌江黎族自治县","澄迈县","定安县","东方","海口","乐东黎族自治县","临高县","陵水黎族自治县","琼海","琼中黎族苗族自治县","三亚","屯昌县","万宁","文昌","五指山","儋州","西沙","南沙","中沙"]); 
...
dsy.add("0_0_18",["固原","石嘴山","吴忠","银川","中卫"]); 
...
dsy.add("0_0_23",["长宁区","卢湾区","杨浦区","吴松区","闵行区","闸北区","虹口区","徐汇区","黄浦区","普陀区","静安区","嘉定区","宝山区","南汇区","奉贤区","松江区","金山区","青浦区","崇明县"]);  
dsy.add("0_0_24",["阿坝藏族羌族自治州","巴中","成都","达州","德阳","甘孜藏族自治州","广安","广元","乐山","凉山彝族自治州","眉山","绵阳","南充","内江","攀枝花","遂宁","雅安","宜宾","资阳","自贡","泸州"]); 
dsy.add("0_0_25",["和平","河东","河西","河北","南开","红桥","塘沽","汉沽","东丽","津南","西青","北辰","武清","宝坻","宁河","静海","蓟县","大港"]); 
...
dsy.add("0_0_30",["万州区","涪陵区","渝中区","大渡口区","江北区","沙坪坝区","九龙坡区","南岸区","北碚区","万盛区","双桥区","渝北区","巴南区","黔江区","长寿区","江津区","合川区","永川区","南川区","綦江县","潼南县","铜梁县","大足县","荣昌县","璧山县","梁平县","城口县","丰都县","垫江县","武隆县","忠县","开县","云阳县","奉节县","巫山县","巫溪县","石柱土家族自治县","秀山土家族苗族自治县","酉阳土家族苗族自治县","彭水苗族土家族自治县"]); 
dsy.add("0_0_31",["香港"]);
dsy.add("0_0_32",["澳门"]);
dsy.add("0_0_33",["高雄","台南","台中","台北","基隆"]);
...
var s=["nationality","province","city"]; 
var opt0 = ["请选择国家","请选择省份","请选择地级市、县"]; 
function setup()
{ 
 for(i=0;i<s.length-1;i++) 
  ...
  change(0);
	...
	var layer=document.getElementById("number1");
	if ( layer!=null ){
		var nation=document.getElementById("number1").innerHTML;
		var province=document.getElementById("number2").innerHTML;
		var city=document.getElementById("number3").innerHTML;
		if ( nation!=null && nation!="" ){
			set(nation, province, city);
		}
	}
 }
